...
class Table:
    ...
    def lines_for_row(self, row_idx: int) -> list[str]:
        ...
        if row_idx < 0:
            ...
            for column in self._column_names:
                ...
                header_underline += "-" * count
                while count < widths[column_number]:
                    header_string += " "
                    count += 1
                    header_underline += "-"
                column_number += 1
                header_string += Table._COLUMN_SEPARATOR
                header_underline += Table._COLUMN_SEPARATOR

            row_result.append(header_string)
            row_result.append(header_underline)
        ...
        count = 0
        max_len = 0
        for entry in self._data[row_idx]:
            add = None
            if self._format_spec[count] == FormatSpec.PREFORMATTED:
                add = entry.split(Table._PREFORMAT_SEPARATOR)
            elif self._format_spec[count] == FormatSpec.WRAPPED:
                add = textwrap.wrap(str(entry), width=widths[count])
            elif self._format_spec[count] == FormatSpec.TRUNCATED:
                print(entry)
                if len(entry) > widths[count]:
                    new_entry = ""
                    for i in range(widths[count] - 3):
                        new_entry += entry[i]
                    new_entry += Table._DOTS
                    add = [new_entry]
                else:
                    add = [entry]
            cols.append(add)
        ...
        for index in range(max_len):
            ...
            for col in cols:
                try:
                    row_result.append(col[index])
                except IndexError:
                    row_result.append(" " * self._column_widths[count])
                count += 1
    ...
    def __str__(self) -> str:
        """Returns a string that, when printed, displays a table no wider than max_width with each
            row containing the values from one element of data. It has the following properties.
                ...
                * The second line of input is a sequence of blocks of head_underline characters,
                    ...
                    character.
                * The remaining lines of the table are the rows of data, each value formatted
        ...
        """
        final = ""
        head_and_underline = self.lines_for_row(-1)
        final += head_and_underline[0] + "\n"
        final += head_and_underline[1] + "\n"
        for row_idx in range(len(self._data)):
            count = 0
            row = self.lines_for_row(row_idx)
            row_count = 0
            for entry in row:
                if ((count == 0 and row_idx != 0) or
                    (count == 0 and row_idx == 0 and row_count != 0)):
                    final += "\n"
                final += entry + Table._COLUMN_SEPARATOR
                spaces = self._column_widths[count] - len(entry)
                i = 0
                while i < spaces:
                    final += " "
                    i += 1
                count += 1
                row_count += 1
                if count == len(self._column_names):
                    count = 0
